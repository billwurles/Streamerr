...
const options = {
	streaming: {
		...
		stdout: {
			desc: 'Standard out (implies --quiet)',
			type: 'boolean'
		}
	},
	simple: {
	...
	advanced: {
		...
		verbose: {
			desc: 'Show torrent protocol details',
			type: 'boolean'
		},
		playlist: {
		...
		'no-quit': {
			desc: 'Don\'t quit when player exits',
			type: 'boolean'
		},
		quit: {
			hidden: true,
...
const commands = [{
	...
	} // Implicitly calls showHelp, as a result middleware is not executed
]
...
async function runDownload(torrentId) {
	...
	torrent.on('done', () => {
		...
		if (argv.onDone) {
			cp.spawn(argv.onDone[0], argv.onDone.slice(1), {
					shell: true
				})
...
function gracefulExit() {
	...
	if (argv.onExit) {
		cp.spawn(argv.onExit[0], argv.onExit.slice(1), {
				shell: true
			})
			.on('error', (err) => fatalError(err))
			.stderr.on('data', (err) => fatalError(err))
